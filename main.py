...
async def fetch_accession(
    terms: str = Query(..., description="Search term to retrieve accession numbers", example="WA-PHL-007327"),
    api_key: str = Query(None, description="User's NCBI API key"),
    timeout: int = Query(REQUEST_TIMEOUT, description="Timeout for requests"),
    num_workers: int = Query(5, description="Number of concurrent workers"),
    max_retries: int = Query(MAX_RETRIES, description="Maximum number of retries per term"),
    request_delay: int | float = Query(REQUEST_DELAY, description="Delay between requests (in seconds)")
) -> dict[str, str]:
    """
    ...
    """
    terms_list = terms.split(",")  # Split terms if there are multiple terms in the query string
    results = await fetch_all_nuccore(
        terms_list, api_key=api_key, timeout=timeout, num_workers=num_workers,
        ...
        while retries < max_retries:
            try:
                ...
                for uid in id_list:
                    ...
                    if accession_result and re.match(ACCESSION_PATTERN, accession_result) and term in title_result:
...
async def fetch_all_nuccore(terms, api_key, timeout=15, num_workers=5, max_retries=MAX_RETRIES, request_delay=REQUEST_DELAY):
    ...
    Parameters:
        terms str: A search term or comma-separated search terms.
    ...
    async with aiohttp.ClientSession() as session:
        workers = [asyncio.create_task(worker(queue, session, results, semaphore, api_key, timeout, max_retries, request_delay)) for _ in range(num_workers)]
...
async def worker(queue, session, results, semaphore, api_key, timeout, max_retries, request_delay):
    ...
    while True:
        ...
        try:
            term, result = await fetch_nuccore(term, session, semaphore, api_key, timeout, max_retries, request_delay)
            results[term] = result
        except Exception as e:
